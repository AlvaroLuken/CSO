...
unsigned int num_bits_tag(){
	...
	tagbits = ADDRS_LENGTH - set_bits - offset_bits;
...
unsigned long find_set(unsigned long address){
   ...
   unsigned int index = (address >> offset_bits) & ((1 << set_bits) -1);
...
unsigned long find_tag(unsigned long address){
 ...
 // unsigned int tag_1 = (address >> (set_bits+offset_bits)) & ((1 << tagbits)-1);
  
	unsigned long tag_1 = (address >> (set_bits+offset_bits));
  return tag_1;
}
...
unsigned long find_offset(unsigned long address){
  ...
  unsigned int offset = (address >> 0) & ((1 << offset_bits)-1);
